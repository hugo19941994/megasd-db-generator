...
def download(platform, file_name):
    ...
    for i in range(10):
        ...
        querystring = {"api_key": API_KEY, "format": "json", "field_list": "guid,name",
                       "filter": f"platforms:{platform}", "offset": offset}
        headers = {
            'Accept': "*/*",
    ...
    with open(f'dbs/{file_name}', 'w') as f:
        querystring = {"api_key": API_KEY, "format": "json",
                       "field_list": "original_release_date,genres,expected_release_year,aliases"}
        headers = {
            'Accept': "*/*",
            'Cache-Control': "no-cache",
            'Accept-Encoding': "gzip, deflate",
        ...
        for r in results:
            ...
            alternative_names = response['results']['aliases']
            ...
            res[r['name']] = {'release_dates': [{'y': date}], 'genres': genres}
            if alternative_names is not None:
                res[r['name']]['alternative_names'] = alternative_names
            print(res[r['name']])
            time.sleep(1)
