...
class XMLGenerator:
    ...
    def generate_zip(self, xml_str):
        # create a ZipFile object
        zip_obj = ZipFile(f"DB_{date.today()}.zip", "w")

        # Add multiple files to the zip
        zip_obj.writestr("db.xml", xml_str)
        zip_obj.writestr("cover_fuzzy_matches.csv", self.cfm.getvalue())
        zip_obj.writestr("info_fuzzy_matches.csv", self.ifm.getvalue())

        # Insert images
        for root, _, files in os.walk("output"):
            for f in files:
                zip_obj.write(os.path.join(root, f))
    ...
    def run(self):
        ...
        self.do_roms(
            "Mega Drive - Genesis", "*.md", "output/Sega - Mega Drive - Genesis/Named_Titles", "dbs/genesis.json", a
        )
        self.do_roms(
            "Master System - Mark III", "*.sms", "output/Sega - Master System - Mark III/Named_Titles", "dbs/ms.json", a
        )
        ...
        self.do_roms(
            "Sega - Mega CD & Sega CD - Datfile (MegaSD).dat",
            "*.cue",
            "output/Sega - Mega CD & Sega CD/Named_Titles",
            "dbs/cd.json",
            a,
        )
    ...
    def crc(file_name):
        ...
        if ext == ".cue":
            ...
            file_name = f"{os.path.dirname(file_name)}/{bin_file}"
            prev = 0
