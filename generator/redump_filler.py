...
def check_missing(game_source):
    # Game source is either a URL or local folder

    # First download the newset Redump DAT and load it
    redump_dat_name = download_redump()
    redump_dict = _dat_to_dict(f"./dats/{redump_dat_name}")

    # The load the custom MegaSD DAT file and the Extras DAT file
    megasd_dict = _dat_to_dict("./dats/Sega - Mega CD & Sega CD - Datfile (MegaSD).dat")
    extras_dict = _dat_to_dict("./dats/Sega - Mega CD & Sega CD - Datfile (EXTRAS).dat")
    ...
    for name in redump_dict:
        if name not in megasd_dict:
            ...
            if game_source.startswith("http"):
                crc = asyncio.run(download_rom(name[:-4], game_source))
            else:
...
async def download_chunk(url, start_byte, end_byte, zipdata, client):
    ...
    end_time = time.time()
    print(f"{start_byte}-{end_byte} took {end_time - start_time!s}")


async def download_rom(name, base_url):
...
def cal_crc(file_io):
    ...
    for a in archive.namelist():
        ...
        if ext == ".cue":
            ...
            bin_file = archive.open(bin_file)
            prev = 0
            bin_file.read(16)
            b = bin_file.read(2048)
